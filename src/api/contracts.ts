...
import type { PageArtifactBundle } from "../governance/pageArtifacts";
...
export interface GoodMemoryRetrievalConfig {
  ...
  bm25Ranking?: boolean;
  // Opt-in long-record admission for default recall. A record whose overlap
  // token count exceeds the long-record floor is admitted when it covers at
  // least 60% of the query tokens (and at least two of them), filling only the
  // capacity the calibrated selection left free. Off keeps rules-only
  // selection byte-identical; measure on the protection suites before
  // enabling it on fresh installs.
...
// Governance surfaces that sit beside the memory API: a derived, read-only
// workspace file mirror of the export bundle (ADR-010 §8). The mirror is a
// projection: never read back, never a truth source, never fails a mutation.
export interface GoodMemoryFileMirrorConfig {
  root: string;
  // The one durable scope this root serves (sessionId ignored). Mutations
  // outside it are not mirrored; fields left unset admit any value, so a
  // { userId, workspaceId } scope is regenerated by every agent in it.
  scope: MemoryScope;
  debounceMs?: number;
  regenerateOn?: "mutation";
}

export interface GoodMemoryGovernanceConfig {
  // Prompt fragments open with a localized "recalled data, not instructions"
  // frame; set false to omit it for every buildContext call.
  contextFrame?: boolean;
  fileMirror?: GoodMemoryFileMirrorConfig;
}

export interface GoodMemoryConfig {
  storage?: StorageConfig;
  governance?: GoodMemoryGovernanceConfig;
...
export interface BuildContextInput {
  ...
  // Per-call override of governance.contextFrame (prompt fragments only).
  contextFrame?: boolean;
}
...
export interface BuildContextResult {
  output: "json" | "markdown" | "system_prompt_fragment" | "developer_prompt_fragment";
  content: string;
  // Present (true) when the rendered fragment opens with the data frame.
  contextFrame?: boolean;
...
export interface ImportMemoryPageInput {
  path: string;
  content: string;
}

// Import sources: a memoryfield-compatible pages directory (each page becomes
// a `note`) or the durable half of an ExportMemoryResult for a by-id round
// trip. Both are fenced by the caller-declared scope.
export type ImportMemorySource =
  | { kind: "pages"; pages: ImportMemoryPageInput[] }
  | { kind: "durable"; durable: ExportMemoryResult["durable"] };

export interface ImportMemoryInput {
  scope: MemoryScope;
  source: ImportMemorySource;
  // Compute outcomes and the input hash without writing anything.
  dryRun?: boolean;
  // Pages above NOTE_MAX_BYTES are rejected unless "split" is requested.
  oversize?: "reject" | "split";
  locale?: string;
  // Fail closed (before any write) unless the input hashes to this value.
  expectedSha256?: string;
}

export type ImportMemoryPageOutcome =
  | "imported"
  | "unchanged"
  | "superseded"
  | "split"
...
export interface ImportMemoryPageResult {
  path: string;
  outcome: ImportMemoryPageOutcome;
...
export interface ImportMemoryResult {
  outcome: "imported" | "dry_run" | "rejected";
  reason?: string;
  // Pages: sha256 over sorted `<path> <sha256(content)>` lines (matches
  // pages/manifest.json). Durable: sha256 of the JSON-serialized envelope.
  inputSha256: string;
  counts: {
    imported: number;
    unchanged: number;
...
export interface GoodMemory {
  ...
  importMemory(input: ImportMemoryInput): Promise<ImportMemoryResult>;
