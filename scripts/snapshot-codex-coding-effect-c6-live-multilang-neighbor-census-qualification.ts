import {
  materializeC6LiveMultiLangNeighborCensusQualification,
...
export interface C6LiveMultiLangNeighborCensusQualificationCliOptions {
  actorFrame: string;
...
export function parseC6LiveMultiLangNeighborCensusQualificationCliOptions(
  args: readonly string[],
): C6LiveMultiLangNeighborCensusQualificationCliOptions {
  const values = new Map<string, string>();
  for (const argument of args) {
    const match = /^--([^=]+)=(.*)$/u.exec(argument);
    if (match === null) {
      throw new Error(
        `invalid C6 neighbor qualification argument ${argument}`,
      );
    }
    const [, name, value] = match;
    if (!OPTION_NAMES.has(name!)) {
      throw new Error(
        `unknown C6 neighbor qualification option --${name}`,
      );
    }
    if (values.has(name!)) {
...
export async function runC6LiveMultiLangNeighborCensusQualificationSnapshotCommand(
  args: readonly string[],
): Promise<{
  boundary: {
  ...
  const options =
    parseC6LiveMultiLangNeighborCensusQualificationCliOptions(args);
  const result =
    await materializeC6LiveMultiLangNeighborCensusQualification({
  ...
  return {
    boundary: {
      acceptedEpisodeCount:
        result.qualification.boundary.acceptedEpisodeCount,
      candidateManifestFrozen:
        result.qualification.boundary.candidateManifestFrozen,
      codexRunReady:
        result.qualification.boundary.codexRunReady,
      status: result.qualification.boundary.status,
    },
    counts: {
...
if (import.meta.main) {
  try {
    const result =
      await runC6LiveMultiLangNeighborCensusQualificationSnapshotCommand(
