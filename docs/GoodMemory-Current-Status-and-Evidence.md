...
- The historical v0.7.3 source contains complete lifecycle and
  ...
  closure is preserved under
  `reports/release/v0.7/v0.7.3-locomo-claim-evidence/`. A later audit found
  that `locomo-v0.7.3-current.json` is a presentation projection, not
  end-to-end runner evidence. It therefore cannot authorize promotion,
  and the retained result is now an internal diagnostic rather than public or
  versioned evidence. A later
  governance-only correction separated the current v0.7.3 README presentation
  from a historical v0.6 projection. Neither projection passes the empty
  end-to-end runner allowlist, so both are now internal diagnostics. Its preregistration,
...
- Built-in write admission is clause-local across `en`, `zh-Hans`, `zh-Hant`,
  ...
  automatic producer. Each built-in also classifies behavioral directives as
  `none`, `one_off`, or `durable`; politeness and pack-recognized imperative
  constructions alone are one-off and abstain before every automatic producer.
  A standing cue can upgrade a behavioral directive but cannot turn an ordinary
  assertion into feedback. Assertion prefixes in mixed
  messages and explicit memory directives, assignments, quoted literals,
  public `feedback()`, and a confirmed `remember: "always"` annotation remain
  eligible authority paths.
...
- RuntimeKit does not persist routine remember/recall operation summaries as
  durable experiences. Recall persists a `verify` experience only when it
  actually emits a verification signal; actionable feedback, maintenance,
  session-end, agent-correction, and tool-outcome records keep their existing
  governed paths. Redacted operation counts remain available in return values
  and the optional external trace sink. A raw behavioral carryover is eligible
  only from a `tool_outcome` experience with a valid safer alternative, an
  explicit stored retrieval profile exactly matching the current call, and a
  valid reference-time boundary. Eligible carryover can independently produce
  context and reports its exact experience in `recordRefs`; routine or legacy
  profile-less experiences fail closed. Behavioral outcomes validate all
  persisted fields before storage and commit evidence plus experience as one
  immutable aggregate: identical `traceId` retries are idempotent, conflicting
  payloads fail, and storage failures propagate to the host recorder.
...
- The `0.7.4` source line has no current or versioned historical benchmark
  ...
  remain internal diagnostics; their original artifacts are retained without
  retrofitting new results.
...
- The current v0.7.4 benchmark surface contains no current or versioned
  ...
  capability descriptor and both README tables are empty. No end-to-end
  benchmark runner is allowlisted, so current and historical promotion both fail
  closed. Stored-answer rescores, legacy projections, and declaration-supplied
  README fragments cannot open that boundary. All remain internal diagnostics.
- ImplicitMemBench Full-300 rerun evidence guard note: `eval:phase-61-full300` rejects ambiguous source/output/run/budget selectors before launching live shards. The 2026-07-06 full-root run `run-phase61-full300-rerun-20260706-codex-current` completed 300 cases with zero failures and measured same-model diagnostic GoodMemory 0.7081666667 versus baseline 0.41. That number and the later gpt-5.4 stored-answer rescore remain internal diagnostics because no end-to-end claim runner is allowlisted.
...
- Public-claim gate note: `gate:public-benchmark-claim` validates declaration shape, metric direction and baseline improvement, answer/judge separation, and the two README table boundaries. Its end-to-end runner allowlist is intentionally empty because the repository has no producer whose answer-generation identity and source closure can be independently verified. Current and versioned historical promotion therefore both fail closed. Declarations cannot provide presentation fragments, diagnostic artifacts cannot unlock promotion, and legacy receipt/projection/binding protocols are not compatibility paths. For `v0.7.4`, strict mode finds no current or versioned historical benchmark evidence; all five declarations are paused boundaries.
- Phase 67 benchmark-prompt rescore evidence note: `eval:official-rescore` rejects ambiguous selectors, source/output overlap, malformed identities and progress rows, source fingerprint drift, judge-model drift, and incomplete judging. Its source reports do not bind a verifiable answer-run identity, so this stored-answer scorer cannot safely derive a public claim; accepting claim fields from CLI would be self-attestation. The command remains an internal rescore tool and emits only `rescore-summary.json`. A future claimable benchmark producer and verifier must be implemented together. LongMemEval runs with gpt-5.4 or gpt-5.5 are explicitly official-prompt-compatible but not directly comparable to published official scores because those models are outside the pinned evaluator model zoo. Stored answers remain separate from derived artifacts, and no rescore becomes a public claim without the claim gate.
- Phase 67 official-rescore cache writer / summary validation note: `eval:official-rescore` serializes progress through the same strict shapes accepted by resume parsing and validates final summaries, scope counts, fingerprints, category aggregates, and stored-answer boundaries before write. Existing LoCoMo and BEAM scores remain internal diagnostic artifacts because the end-to-end runner allowlist is empty; refreshed LongMemEval artifacts preserve contaminated provenance only.
...
- Phase 58: the historical Raw Enactment Compiler gate at `run-20260504183000` recorded raw `41 / 50` using the retired transcript/archive inference path. Current typed-outcome regeneration uses `run-phase58-typed-outcome-current`: all 50 raw cases must have zero candidates and zero selections, its deterministic 28 / 50 memory-independent smoke-answer passes are not carryover evidence, and distilled remains at least `48 / 50`. Current fail-closed gate artifacts use `run-20260813083000`, leaving the historical accepted run immutable.
- Phase 59: Phase 59 is the historical Generalized Raw Executor Cleanup slice; failed/preferred operations, `reports/eval/fallback/phase-59/run-phase59-fallback-current/report.json`, `reports/eval/fallback/phase-59/run-phase59-fallback-current/raw-diagnostics.json`, `reports/quality-gates/phase-59/run-20260504193000/phase-59-quality-gate.json`, `docs/archive/quality-gates/GoodMemory-Phase-59-Quality-Gate.md`, `goodmemory-raw-experience` at `58 / 60`, raw `90 / 200`, raw `88 / 200`, raw at least `115 / 200`, distilled `151 / 200`. Those raw scores used the retired transcript/archive inference path and are not evidence for the current RuntimeKit. Current raw carryover accepts only persisted, profile-matched `tool_outcome` records with an observed safer alternative; transcript-only Phase 59 fixtures intentionally produce zero raw exemplars, while the distilled profile remains executable. Current fail-closed gate regenerations use `run-20260813090000`, leaving the historical accepted run immutable.
